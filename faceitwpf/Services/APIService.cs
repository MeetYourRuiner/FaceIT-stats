...
namespace faceitwpf.Services
{
    public class APIService : IAPIService
    {
        ...
        {
            var response = await _client.GetAsync($"https://api.faceit.com/match-history/v5/players/{playerId}/history/?to=2021-02-25T21:43:00+0000&from=2020-02-25T21:43:00+0000&playerId={playerId}");
            if (response.StatusCode != HttpStatusCode.OK)
                throw new Exception("Failed to get match history");
            string json = await response.Content.ReadAsStringAsync();
            var matchList = JsonConvert.DeserializeObject<List<Match>>(json);
